...
public class TimeEntryAdapterBU extends ResourceCursorAdapter {
    ...
    public TimeEntryAdapterBU(Context context, Cursor cursor) {
        ...
        if (entries.isEmpty()) {
            ...
            for (Entry entryitem : entries) {
                ...
                if (!TextUtils.isEmpty(entryitem.getHouseholder()) || !TextUtils.isEmpty(entryitem.getNotes()) || entryitem.pubs.size() > 0) {
                    ...
                    LayoutParams lp3 = new LayoutParams(LayoutParams.MATCH_PARENT, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, context.getResources().getDisplayMetrics()));
